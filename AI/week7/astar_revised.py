		...
		for neighbour, e_cost in g.adj_list[curr]:
			...
			if neighbour not in explored:
				...
				for i in range(len(open.queue)):
					# It doesn't make sense to compare the previously stored g_cost with f_cost
					# if(open.queue[i][0] == neighbour and open.queue[i][2]>f_cost):
					# if(open.queue[i][0] == neighbour and open.queue[i][2]>g_cost): - this is just ucs
					if(open.queue[i][0] == neighbour and (open.queue[i][2] + h[open.queue[i][0]])>f_cost):
